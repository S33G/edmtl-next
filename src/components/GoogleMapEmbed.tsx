...
export default function GoogleMapEmbed({
  ...
  const { latitude, longitude } = siteConfig.contact;

  // Check if we have valid coordinates
  if (!latitude || !longitude || !siteConfig.google.mapsApiKey || siteConfig.google.mapsApiKey === 'PLACEHOLDER_FOR_GOOGLE_MAPS_API_KEY') {
    return (
  ...
  // Create the embed URL with the EDMTL location and service radius
  const getEmbedUrl = (): string => {
    const baseUrl = `https://www.google.com/maps/embed/v1/place`;
    const params = new URLSearchParams({
      key: siteConfig.google.mapsApiKey,
